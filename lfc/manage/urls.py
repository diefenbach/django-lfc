...
urlpatterns = patterns('lfc.manage.views',
    ...
    url(r'^load-object-children/(?P<child_id>\d+)', "load_object_children", name="lfc_load_object_children"),
...
if getattr(settings, "LFC_MANAGE_META_DATA", True):
    urlpatterns += patterns('lfc.manage.views',
        url(r'^save-meta-data/(?P<id>\d+)$', "object_meta_data", name="lfc_save_meta_data"),
    )

# seo
if getattr(settings, "LFC_MANAGE_SEO", True):
    urlpatterns += patterns('lfc.manage.views',
        url(r'^save-seo/(?P<id>\d+)$', "object_seo_data", name="lfc_save_seo"),
    )

# history
if getattr(settings, "LFC_MANAGE_HISTORY", True):
    urlpatterns += patterns('lfc.manage.views',
        # history
...
# multi language
if getattr(settings, "LFC_MULTILANGUAGE", True):
    urlpatterns += patterns('lfc.manage.views',
...
# utils
if getattr(settings, "LFC_MANAGE_UTILS", True):
    urlpatterns += patterns('lfc.manage.views',
...
if getattr(settings, "LFC_MANAGE_APPLICATIONS", True):
    urlpatterns += patterns('lfc.manage.views',
        # applications
...
# workflows
if getattr(settings, "LFC_MANAGE_WORKFLOWS", True):
    urlpatterns += patterns('lfc.manage.views',
...
# users, groups & roles
if getattr(settings, "LFC_MANAGE_USERS", True):
    urlpatterns += patterns('lfc.manage.views',
        # users
